...
import type { ActionResult, AuthResult, StoredSession } from "../types/index.js";
...
function toStoredSession(raw: RawAuthResponse): StoredSession {
  return {
...
export async function signInWithPassword(email: string, password: string): Promise<AuthResult> {
  try {
    ...
    const session = toStoredSession((await response.json()) as RawAuthResponse);
...
async function refreshSession(session: StoredSession): Promise<StoredSession | null> {
  try {
    ...
    const refreshed = toStoredSession((await response.json()) as RawAuthResponse);
