 ...
 * same backend, same publication/RLS setup the web app already requires,
 * so no additional Supabase configuration is needed for the desktop app.
 *
 ...
 * after a reconnect). Day to day, every change — including this window's
 * own mutations — arrives through the subscription below, so multiple
 * desktop windows (and the web app) stay in sync without polling.
 */
